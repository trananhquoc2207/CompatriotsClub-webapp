...
class EnglishCenterApi {
    get = (params) => {
        return axiosClient.get('/CoSo', { params });
    };

    getID = id => {
        return axiosClient.get(`/CoSo/${id}`)
            .then(response => {
                if (response)
                    return response;
                throw response;
...
const englishCenterApi = new EnglishCenterApi();
export default englishCenterApi;
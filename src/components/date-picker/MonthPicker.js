...
const MonthPicker = (props) => {
  ...
  const onChange = (d) => {
    ...
    if (typeof onChangeProps === 'function') {
      if (range) {
        onChangeProps(
          format ? `${d.getFullYear()}-${d.getMonth() + 1}-01` : new Date(d.getFullYear(), d.getMonth() + 1, 1),
          format ? `${d.getFullYear()}-${d.getMonth() + 1}-${new Date(d.getFullYear(), d.getMonth() + 1, 0).getDate()}` : new Date(d.getFullYear(), d.getMonth() + 1, 0)
        );
      } else {

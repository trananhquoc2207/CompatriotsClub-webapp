...
const Request = ({ data }) => {
    return (
        <Wrapper>
            ...
            <div className="body">
                <div className="body__title">ĐƠN XIN NGHỈ PHÉP</div>
                ...
                <div className="body__content">
                    ...
                    <div className="line">Nay tôi làm đơn này kính xin <span class="font-weight-bold">Ban Giám đốc</span> chấp thuận cho tôi dược nghỉ phép trong thời gian <span className="font-weight-bold"></span> kể từ <span className="font-weight-bold">ngày {data?.beginDate ? dayJS(data.beginDate).format('DD/MM/YYYY') : '-'}</span> đến hết <span className="font-weight-bold">ngày {data?.endDate ? dayJS(data.endDate).format('DD/MM/YYYY') : '-'}</span></div>
                    <div className="line">Vì lí do: <span className="ml-1">{data?.reason ?? '-'}</span></div>
                    <div className="line">Tôi hứa sẽ cập nhật đầy đủ nội dung công tác trong thời gian vắng.</div>

        ...
        <tbody>
            {(!data || data.length < 1)
                ...
                : data.map((item, idx) => {
                    ...
                    return (
                        <tr className={classNames({ 'bg-green': isChecked })} key={'certificate_' + idx}>
                            <td>
...
const Certificate = (props) => {
    ...
    const handleEdit = event => {
        ...
        setModalEdit({ ...modalEdit, isShow: true, certificate: data[id] });
    }
    ...
    const reload = (type = 'add') => {
        ...
        // Modal
        setModalAdd(false);
        setModalEdit({ isShow: false, certificate: { id: -1, code: '', name: '' } });
    ...
    const editCertificate = async (payload) => {
        if (payload.name !== modalEdit.certificate.name) {
            /* Request */
            const params = {
                id: modalEdit.certificate.id,
                tenBangCap: payload.name
            }
    ...
    const deleteCertificate = async (id) => {
        if (id === 'all') {
            ...
            data.forEach(item => {
                if (item.isChecked) {
                    requests.push(certificateApi
                        .delete(item.id)
                        .catch(() => { notify('danger', props.t('Can\'t delete certificate #') + item.code) }));
                }
            });
            ...
            notify('danger', props.t('Certificate ID not found'));
    }
                                        ...
                                        <Col sm={6} md={4} xl={3}>
                                            <Modal size={'sm'} isOpen={modalAdd}>
                                                <ModalHeader toggle={() => setModalAdd(false)}>
                                                    <span className="font-weight-bold">{props.t('Add new certificate')}</span>
                                                </ModalHeader>
                                                <ModalBody>
                                                    <Form onSubmit={handleSubmit(addCertificate)}>
                                                        <Row className="mb-1">
                                                            <Col>
                                                                <span style={{ fontSize: '15px', fontWeight: '400' }}>{props.t('Certificate name')}</span>
                                                            </Col>
                                                        </Row>
                                                        <Row className="mb-3">
                                                            <Col>
                                                                <Input
                                                                    ...
                                                                    placeholder={props.t('Enter certificate name')}
                                                                    innerRef={
                                                                        register({
                                                                            required: props.t('Certificate name not entered')
                                                                        })
                                                                    } />
                                            ...
                                            <Modal size={'sm'} isOpen={modalEdit.isShow}>
                                                <ModalHeader toggle={() => setModalEdit({ isShow: false, certificate: { id: -1, name: '' } })}>
                                                    <span className="font-weight-bold">{props.t('Edit certificate') + ' #' + (modalEdit.certificate.code !== '' ? modalEdit.certificate.code : 'ID')}</span>
                                                </ModalHeader>
                                                <ModalBody>
                                                    <Form onSubmit={handleSubmit(editCertificate)}>
                                                        <Row>
                                                            <Col>
                                                                <FormGroup>
                                                                    <Label for="name">{props.t('Certificate name')}</Label>
                                                                    <Input
                                                                        name="name"
                                                                        className={errors.name ? 'is-invalid' : ''}
                                                                        placeholder={props.t('Enter certificate name')}
                                                                        defaultValue={modalEdit.certificate.name}
                                                                        innerRef={
                                                                            register({
                                                                                required: props.t('Certificate name not entered')
                                                                            })
                                                        ...
                                                        <Button type="button" className="ml-2" color="secondary" onClick={() => setModalEdit({ isShow: false, certificate: { id: -1, code: '', name: '', allowance: 0 } })}>{props.t('Cancel')}</Button>
                                                    </Form>
                                                </ModalBody>
                                            </Modal>
                                            <Modal size={'sm'} isOpen={modalDelete.isShow} style={{ top: '100px', maxWidth: '350px' }}>
                                                ...
                                                <ModalBody className="justify-content-center" style={{ textAlign: 'center', paddingTop: '0', paddingBottom: '2rem' }}>
                                                    <Button type="button" color="danger" size="md" onClick={() => { deleteCertificate(modalDelete.id) }}>
                                                        {props.t('Accept')}
                                                    </Button>
                                        ...
                                        <Table className="table table-wrapper table-hover table-nowrap mb-0">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th className="text-left">{props.t('Certificate name')}</th>
...
export default withNamespaces()(Certificate);
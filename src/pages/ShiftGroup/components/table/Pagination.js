...
const Pagination = (props) => {
  ...
  const calulate = (currentPage, totalSizes) => {
    ...
    for (const i of range) {
      if (flag) {
        if (i - flag === 2) { rangeWithDots.push(flag + 1); } else if (i - flag !== 1) { rangeWithDots.push('...'); }
      }
  ...
  return (
    <Wrapper>
      <div className="pagination">
        <div className="item" onClick={() => { onChange(1); }}><i className="bx bx-arrow-to-left" /></div>
        <div className="item" onClick={() => { onChange(pageIndex - 1); }}><i className="bx bx-left-arrow-alt" /></div>
        {((range || []).map((l, i) => (
          <div key={`pagination_${i}`} className={classnames('item', { 'item-active': l === pageIndex })} onClick={() => { onChange(l); }}>{l}</div>
        )))}
        <div className="item" onClick={() => { onChange(pageIndex + 1); }}><i className="bx bx-right-arrow-alt" /></div>
        <div className="item item-last" onClick={() => { onChange(pageLast); }}><i className="bx bx-arrow-to-right" /></div>
        <div className="total">

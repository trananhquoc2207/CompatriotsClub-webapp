...
const DeleteScheduleLoopModal = ({ data, onRefresh, onClose }) => {
  const onSubmit = useCallback(async () => {
    try {
      await httpClient.callApi({
        method: 'PUT',
        url: apiLinks.scheduleGroup.deleteScheduleLoop(data.id, data.scheduleLoopConfigurationId),
      });
      onClose();
      notify('success', 'Đã xóa lịch làm việc.');
      onRefresh();
    } catch (error) {
    ...
    <Modal size="sm" isOpen={Boolean(data)} style={{ top: '100px', maxWidth: '350px' }}>
      <ModalHeader style={{ display: 'block', border: 'none', paddingBottom: '.5rem' }}>
        ...
        <p style={{ textAlign: 'center', color: '#999', whiteSpace: 'pre-line', fontSize: '14px' }}>
          Bạn có thật sự muốn xóa lịch làm việc hiện tại của nhóm này ?
        </p>
      </ModalHeader>
...
export default DeleteScheduleLoopModal;

...
const CreateModal = ({ open, onClose, onRefresh }) => {
  ...
  useEffect(() => {
    ...
    register('members', { required: 'Bắt buộc chọn nhân viên phụ trách' });
    ...
    <StyledModal size="md" isOpen={open}>
      <Form onSubmit={handleSubmit((d) => onSubmit(d))}>
        ...
        <ModalBody>
          ...
          <StyledCard>
            <CardHeader onClick={() => setAccordionContract(!accordionContract)}>
              <span className={classnames('title', { 'title__active': accordionContract })}>Nhân viên phụ trách dự án</span>
            </CardHeader>
            <CardBody className={classnames({ 'disabled': !accordionContract })}>
              ...
              <Row>
                <Col xs={12}>
                  <FormGroup>
                    <Label className={classnames({ 'error': !!errors.members })}>Nhân viên phụ trách dự án</Label>
                    <AsyncSelect
                      cacheOptions
                      defaultOptions
                    ...
                    />
                    {(errors?.members ?? false) && <FormFeedback>{errors?.members?.message ?? ''}</FormFeedback>}
                  </FormGroup>
                </Col>
              </Row>
